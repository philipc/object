...
pub trait ReadRef<'a>: Clone + Copy {
    ...
    ///
    /// Checks that the offset and size can be satisfied.
    fn read_bytes_at(self, offset: u64, size: u64) -> Result<&'a [u8]>;

    /// Get a reference to a `u8` slice at the given offset, and update the offset.
    ///
    /// Errors include if the offset or size are outside the block of data.
    ...
    ///
    /// Checks that the offset, size, and alignment can be satisfied.
