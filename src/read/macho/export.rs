    ...
    R: ReadRef<'data>,
{
    pub(super) fn new(file: &'file MachOFile<'data, Mach, R>) -> Result<Self> {
        ...
        let image_base = file
            ...
            .map(|s| s.segment.vmaddr(endian).into())
            .unwrap_or(0);
        ...
        while let Some(command) = commands.next()? {
            ...
            } else if let Some(command) = command.dyld_exports_trie()? {
                exports_trie = Some(command);
            }
        }
        let internal = if let Some(exports_trie) = exports_trie {
            let data = file
                .linkedit_data
                .read_error("Missing Mach-O linkedit segment")?;
            let iter = exports_trie.exports_trie(endian, data.0)?;
            ...
            let data = file
                .linkedit_data
                .read_error("Missing Mach-O linkedit segment")?;
            let iter = dyld_info.exports_trie(endian, data.0)?;
            MachOExportIteratorInternal::ExportsTrie {
                file,
                image_base,
                libraries: None,
        ...
        } else if let Some(dysymtab) = dysymtab {
            let iter = file
                .symbols
                .symbols()
            ...
            MachOExportIteratorInternal::None
        };
    ...
    fn next(&mut self) -> Result<Option<Export<'data>>> {
        match &mut self.internal {
                ...
                let target = match symbol.data() {
                    ExportData::Regular { address } => match flags.kind() {
                        ...
                        macho::EXPORT_SYMBOL_FLAGS_KIND_THREAD_LOCAL => {
                            ExportTarget::ThreadLocalDescriptor {
                    ...
                    ExportData::Reexport {
                        ...
                        let library = usize::try_from(dylib_ordinal.wrapping_sub(1))
                            .ok()
                    ...
                    let target = match symbol.n_type().typ() {
                        ...
                        macho::N_SECT => {
                            if symbol.n_desc(endian).contains(macho::N_SYMBOL_RESOLVER) {
                                ...
                                {
                                    ExportTarget::ThreadLocalDescriptor { address: value }
                                } else {
            ...
            MachOExportIteratorInternal::None => Ok(None),
        }
    }
}
