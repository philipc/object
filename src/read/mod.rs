...
impl<'data> Export<'data> {
    ...
    ///
    /// This is a normalized flag that is derived from the format-specific flags.
    #[inline]
...
impl<'data> fmt::Debug for ExportTarget<'data> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExportTarget::ThreadLocalOffset { offset } => {
                f.debug_struct("Tls").field("offset", offset).finish()
            }
            ExportTarget::ThreadLocalDescriptor { address } => f
                .debug_struct("TlsDescriptor")
