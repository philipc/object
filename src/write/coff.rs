    ...
    pub(crate) fn coff_write(&self, buffer: &mut dyn WritableBuffer) -> Result<()> {
        ...
        for (index, section) in self.sections.iter().enumerate() {
            ...
            if len != 0 {
                ...
                buffer.write_slice(section.data.as_slice());
            }
        ...
        for (index, symbol) in self.symbols.iter().enumerate() {
            ...
            match symbol.kind {
                SymbolKind::File => {
                    ...
                    buffer.write_slice(&symbol.name);
        ...
        buffer.write_slice(&u32::to_le_bytes(strtab_len as u32));
        buffer.write_slice(&strtab_data);
